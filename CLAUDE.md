...
- **Inspecter le schéma ou les policies de la base passe par le MCP supabase en lecture seule** (déclaré dans `.mcp.json`, flag `--read-only`), jamais par `SUPABASE_SERVICE_ROLE_KEY` lu depuis `.env.local`. Cette clé contourne la RLS et ouvre la prod en écriture : elle n'a aucune raison de servir à un outil d'inspection, et l'emprunter pour « juste lire » normalise un accès qui, au premier verbe HTTP près, devient destructif. Si le MCP n'est pas connecté dans la session, le dire et attendre — ne pas se rabattre sur la clé. Le schéma versionné dans `supabase/migrations/` est la référence à consulter en premier ; le MCP sert à confirmer l'état réel de la prod (voir `docs/audit-schema-2026-09.md` pour un exemple d'écart spec/base).
