...
export async function cloneTemplateToAthlete({ templateProgramId, templateTitle, templateActivityType, athleteId, coachId, groupId = null, batchId = null }) {
  ...
  // recommended_sessions_per_week/min_hours_between_sessions suivent la copie pour la même raison
  // qu'elles existent : sans elles, le conseil de rythme du coach n'arrive jamais jusqu'au sportif,
  // et le plafond de choix des jours retombe sur une valeur de repli (voir docs/audit-schema-2026-09.md).
  const { data: sourceProgram } = await supabase.from('programs')
    .select('goal, level, equipment, duration_weeks, recommended_sessions_per_week, min_hours_between_sessions')
